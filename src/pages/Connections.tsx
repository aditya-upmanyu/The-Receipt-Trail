/**
 * Connections Page
 * Visual network graph showing relationships between receipts
 */

import { useState, useMemo } from "react";
import { Network } from "lucide-react";
import type { Receipt, LifeMoment } from "../types/index";
import { getCategoryColor } from "../utils/helpers";

interface ConnectionsProps {
  moments: LifeMoment[];
}

interface GraphNode {
  id: string;
  type: string;
  label: string;
  x: number;
  y: number;
  connections: number;
  data: Receipt | LifeMoment;
}

export function Connections({ moments }: ConnectionsProps) {
  const [selectedNode, setSelectedNode] = useState<GraphNode | null>(null);

  // Build simplified graph from moments (not all receipts)
  const graphData = useMemo(() => {
    const nodes: GraphNode[] = [];
    const edges: Array<{ source: string; target: string; strength: number }> = [];

    // Use moments as nodes for better visualization
    const topMoments = moments.slice(0, 30); // Limit for performance

    topMoments.forEach((moment, i) => {
      const angle = (i / topMoments.length) * 2 * Math.PI;
      const radius = 200;
      const centerX = 400;
      const centerY = 300;

      nodes.push({
        id: moment.id,
        type: "moment",
        label: moment.title,
        x: centerX + radius * Math.cos(angle),
        y: centerY + radius * Math.sin(angle),
        connections: moment.connections.length,
        data: moment,
      });
    });

    // Add edges between connected moments
    for (let i = 0; i < topMoments.length; i++) {
      for (let j = i + 1; j < topMoments.length; j++) {
        const m1 = topMoments[i];
        const m2 = topMoments[j];

        // Check if they share receipts or are close in time
        const sharedReceipts = m1.receiptIds.filter((id) => m2.receiptIds.includes(id)).length;
        const timeDiff = Math.abs(m1.startTime.getTime() - m2.startTime.getTime());
        const hoursDiff = timeDiff / (1000 * 60 * 60);

        if (sharedReceipts > 0 || hoursDiff < 24) {
          edges.push({
            source: m1.id,
            target: m2.id,
            strength: sharedReceipts > 0 ? 2 : 1,
          });
        }
      }
    }

    return { nodes, edges };
  }, [moments]);

  if (moments.length === 0) {
    return (
      <div className="min-h-screen bg-[#05070B] flex items-center justify-center p-6">
        <div className="text-center max-w-md">
          <Network className="w-16 h-16 text-[#94A3B8] mx-auto mb-4" />
          <h2 className="text-2xl font-bold text-[#E8F1FF] mb-2">NO CONNECTIONS YET</h2>
          <p className="text-[#94A3B8]">
            Not enough receipts to detect meaningful connections. Keep exploring!
          </p>
        </div>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-[#05070B]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 className="text-3xl font-bold text-[#E8F1FF] mb-2">Connections</h1>
        <p className="text-[#94A3B8] mb-8">
          {graphData.nodes.length} moments · {graphData.edges.length} connections
        </p>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* Graph visualization */}
          <div className="lg:col-span-2">
            <div className="bg-[#080B12] border border-[#2e303a] rounded-lg p-6 overflow-hidden">
              <svg
                viewBox="0 0 800 600"
                className="w-full h-auto"
                style={{ minHeight: "500px" }}
              >
                {/* Edges */}
                <g>
                  {graphData.edges.map((edge, i) => {
                    const source = graphData.nodes.find((n) => n.id === edge.source);
                    const target = graphData.nodes.find((n) => n.id === edge.target);
                    if (!source || !target) return null;

                    return (
                      <line
                        key={i}
                        x1={source.x}
                        y1={source.y}
                        x2={target.x}
                        y2={target.y}
                        stroke="#2e303a"
                        strokeWidth={edge.strength}
                        opacity={selectedNode ? (selectedNode.id === source.id || selectedNode.id === target.id ? 0.6 : 0.2) : 0.4}
                      />
                    );
                  })}
                </g>

                {/* Nodes */}
                <g>
                  {graphData.nodes.map((node) => {
                    const isSelected = selectedNode?.id === node.id;
                    const isConnectedToSelected = selectedNode
                      ? graphData.edges.some(
                          (e) =>
                            (e.source === selectedNode.id && e.target === node.id) ||
                            (e.target === selectedNode.id && e.source === node.id)
                        )
                      : false;

                    const opacity = !selectedNode || isSelected || isConnectedToSelected ? 1 : 0.3;

                    return (
                      <g
                        key={node.id}
                        onClick={() => setSelectedNode(isSelected ? null : node)}
                        onKeyDown={(e) => {
                          if (e.key === "Enter" || e.key === " ") {
                            e.preventDefault();
                            setSelectedNode(isSelected ? null : node);
                          }
                        }}
                        tabIndex={0}
                        role="button"
                        aria-label={`Select ${node.label}`}
                        style={{ cursor: "pointer", outline: "none" }}
                        className="focus:ring-2 focus:ring-cyan-500 rounded-full"
                        opacity={opacity}
                      >
                        <circle
                          cx={node.x}
                          cy={node.y}
                          r={8 + node.connections}
                          fill={node.type === "moment" ? "#06B6D4" : getCategoryColor("music")}
                          stroke={isSelected ? "#E8F1FF" : "none"}
                          strokeWidth={isSelected ? 2 : 0}
                        />
                        {isSelected && (
                          <text
                            x={node.x}
                            y={node.y - 15}
                            textAnchor="middle"
                            fill="#E8F1FF"
                            fontSize="12"
                            fontWeight="500"
                          >
                            {node.label.slice(0, 20)}
                          </text>
                        )}
                      </g>
                    );
                  })}
                </g>
              </svg>
            </div>
          </div>

          {/* Side panel */}
          <div className="lg:col-span-1">
            <div className="bg-[#080B12] border border-[#2e303a] rounded-lg p-6 sticky top-8">
              {selectedNode ? (
                <div>
                  <h3 className="text-lg font-semibold text-[#E8F1FF] mb-4">
                    {selectedNode.label}
                  </h3>

                  {selectedNode.type === "moment" && (
                    <div className="space-y-4">
                      <div>
                        <p className="text-sm text-[#94A3B8] mb-1">Type</p>
                        <p className="text-[#E8F1FF]">Moment</p>
                      </div>

                      <div>
                        <p className="text-sm text-[#94A3B8] mb-1">Receipts</p>
                        <p className="text-[#E8F1FF]">
                          {(selectedNode.data as LifeMoment).receipts.length} connected
                        </p>
                      </div>

                      <div>
                        <p className="text-sm text-[#94A3B8] mb-1">Connections</p>
                        <p className="text-[#E8F1FF]">{selectedNode.connections}</p>
                      </div>

                      <div>
                        <p className="text-sm text-[#94A3B8] mb-2">Summary</p>
                        <p className="text-sm text-[#E8F1FF]">
                          {(selectedNode.data as LifeMoment).summary}
                        </p>
                      </div>
                    </div>
                  )}

                  <button
                    type="button"
                    onClick={() => setSelectedNode(null)}
                    className="mt-6 w-full px-4 py-2 bg-[#0D111A] hover:bg-[#16171d] text-[#E8F1FF] rounded-lg transition-colors"
                  >
                    Deselect
                  </button>
                </div>
              ) : (
                <div className="text-center">
                  <Network className="w-12 h-12 text-[#94A3B8] mx-auto mb-4" />
                  <p className="text-[#94A3B8]">Click on a node to see details</p>
                </div>
              )}
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
